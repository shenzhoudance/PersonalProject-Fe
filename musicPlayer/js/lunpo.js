...
var bindEventSlide = function() {
	...
	bindAll(selector, 'click', function(event){
		...
		log(toPic)
		if(toPic.classList.contains('prePic')){
				log('1')
				log('nLunpo1' + nLunpo)
				var pre = -1
				nLunpo += pre
				nLunpo = (nLunpo + onlineReList.length) % onlineReList.length
				changePos(nLunpo)
		}else if (toPic.classList.contains('nextPic')) {
				log('-1')
				log('nLunpo3' + nLunpo)
				var next = 1
				nLunpo += next
				nLunpo %= onlineReList.length
				changePos(nLunpo)
		}
	})
}
